...
namespace Common
{
    ...
    public:
        ...
        {
            std::string time_str;
            std::cerr << Common::getCurrentTimeStr(&time_str) << " flushing and closing Logger for file: " << file_name_ << "\n";
            ...
            logger_thread_->join();
            file_.close();
            std::cerr << Common::getCurrentTimeStr(&time_str) << " flushing and closing Logger for file: " << file_name_ << " exiting...\n";
        }
        ...
        auto log(const char *s) noexcept
        {
            while (*s)
            {
                if (*s == '%')
                {
                    ...
                    {
                        FATAL("missing arguments for log()");
                    }
                }
                pushValue(*s++);
            }
